...
class MCTS:
    ...
    def _eval_new(self, states):
        obs = self.env.get_obs_for_states(states)
        priors = self._net.step(obs, self.device_id)
        priors = priors.detach().cpu().numpy()
        cheated_priors = np.zeros(priors.shape)
        n_graphs = priors.shape[0]
        for i in range(n_graphs):
            candidates = []
            # Are there any terminals not considered?
            for v in states[i]["graph"].T:
                if states[i]["visited"][v-1]==0:
                    candidates.append(v-1)
            # Are there any steiner nodes not considered?
            for v in states[i]["graph"].steiner_nodes:
                if states[i]["visited"][v-1]==0:
                    candidates.append(v-1)
            # if there is a candidate, return one random candidate, otherwise return some non visited node
            if len(candidates)>0:
                random.shuffle(candidates)
                cheated_priors[i][candidates[0]] = 1.0
            else:
                for j in range(len(states[i]["visited"])):
                    if states[i]["visited"][j]==0:
                        cheated_priors[i][j] = 1.0
                        break
        #print("states[0].graph.T", states[0]["graph"].T)
        #print("states[0].graph.steiner_nodes", states[0]["graph"].steiner_nodes)
        #print("states[0].tour", states[0]["tour"])
        #print("states[0].visited", states[0]["visited"])
        #print("priors", priors)
        #print("type(priors)", type(priors))
        #print("shape(priors)", priors.shape)
        return cheated_priors
